...
const DOM = {
  ...
  // Modal: Tambah Ketua
  ketuaModal:        $('#ketuaModal'),
  ketuaModalClose:   $('#ketuaModalClose'),
  ketuaModalCancel:  $('#ketuaModalCancel'),
  ketuaDisplayId:    $('#ketuaDisplayId'),
  ketuaNama:         $('#ketuaNama'),
  ketuaShortName:    $('#ketuaShortName'),
  ketuaKelompok:     $('#ketuaKelompok'),
  ketuaPassword:     $('#ketuaPassword'),
  btnSimpanKetua:    $('#btnSimpanKetua'),
...
/* ----- Modal: Tambah Ketua Kelompok ----- */
function openKetuaModal() {
  // Isi dropdown kelompok yang BELUM punya ketua (ketuaId masih placeholder/'-')
  const opsi = DATA.kelompok
    .map(k => `<option value="${k.id}">${k.nama}${k.ketua && k.ketua !== '-' ? ` (saat ini: ${k.ketua})` : ''}</option>`)
    .join('');
  DOM.ketuaKelompok.innerHTML = `<option value="">Pilih kelompok...</option>${opsi}`;

  // Reset form
  DOM.ketuaDisplayId.value = '';
  DOM.ketuaNama.value = '';
  DOM.ketuaShortName.value = '';
  DOM.ketuaPassword.value = '';

  DOM.ketuaModal.hidden = false;
  document.body.style.overflow = 'hidden';
  DOM.ketuaDisplayId.focus();
}

function closeKetuaModal() {
  DOM.ketuaModal.hidden = true;
  document.body.style.overflow = '';
}

async function submitKetuaForm() {
  const displayId   = DOM.ketuaDisplayId.value.trim().toUpperCase();
  const nama        = DOM.ketuaNama.value.trim();
  const shortName   = DOM.ketuaShortName.value.trim();
  const kelompokId  = DOM.ketuaKelompok.value;
  const password    = DOM.ketuaPassword.value;

  if (!displayId || !nama || !shortName || !kelompokId || !password) {
  ...
  DOM.btnSimpanKetua.disabled = true;
  DOM.btnSimpanKetua.textContent = 'Menyimpan...';

  const result = await window.RIHLAH_DB.createKetuaKelompok({
    displayId, nama, shortName, password, kelompokId,
  });

  DOM.btnSimpanKetua.disabled = false;
  DOM.btnSimpanKetua.textContent = 'Simpan';
  ...
  showToast('success', 'Ketua Kelompok Ditambahkan', `${nama} berhasil ditetapkan sebagai ketua.`);
  closeKetuaModal();

  // Refresh data kelompok dari server supaya ketuaList & dropdown ter-update
...
function initSettingsActions() {
  ...
  // Tambah Ketua Kelompok
  const btnTambah = $('#btnTambahKetua');
  if (btnTambah) {
    btnTambah.addEventListener('click', openKetuaModal);
  }
}
...
function initEventListeners() {
  ...
  // --- Modal: Tambah Ketua Kelompok ---
  DOM.ketuaModalClose?.addEventListener('click',  closeKetuaModal);
  DOM.ketuaModalCancel?.addEventListener('click', closeKetuaModal);
  DOM.ketuaModal?.addEventListener('click', (e) => {
    if (e.target === DOM.ketuaModal) closeKetuaModal();
  });
  DOM.btnSimpanKetua?.addEventListener('click', submitKetuaForm);

  // Escape closes modal
  document.addEventListener('keydown', (e) => {
    if (e.key === 'Escape' && !DOM.alertModal.hidden) closeAlertModal();
    if (e.key === 'Escape' && DOM.ketuaModal && !DOM.ketuaModal.hidden) closeKetuaModal();
  });
